...
async function updatePlayers(collection) {
  // Load all player data and filter relevant players
  let allPlayerData = await loadPlayers();
  let filteredPlayerData = await filterPlayers(allPlayerData); // Get filtered players
  let { tradeValueTable, tradeMap } = await loadTradeData();

  // Loop over the filtered players
  for (let currentPlayer of filteredPlayerData) {
    ...
    // Load projections and stats
    const weeklyProj = await load(`https://api.sleeper.com/projections/nfl/player/${currentPlayer.player_id}?season_type=regular&season=2024&grouping=week`) || {};
    const seasonProj = await load(`https://api.sleeper.com/projections/nfl/player/${currentPlayer.player_id}?season_type=regular&season=2024&grouping=season`) || {};
    const weeklyStats = await load(`https://api.sleeper.com/stats/nfl/player/${currentPlayer.player_id}?season=2024&season_type=regular&grouping=week`) || {};
    const seasonStats = await load(`https://api.sleeper.com/stats/nfl/player/${currentPlayer.player_id}?season=2024&season_type=regular&grouping=season`) || {};
    ...
    const tradeValue = parseInt(
      tradeMap.has(currentName)
        ? tradeValueTable[tradeMap.get(currentName)][8]
        : 0
    );
    ...
    let player = {
      name: `${seasonProj?.player?.first_name ?? ""} ${seasonProj?.player?.last_name ?? ""}`,
      adp: adp ?? 0,
      ...
      statsPositionalRanking: seasonStats?.stats?.pos_rank_ppr ?? 999,
      statsOverallRanking: seasonStats?.stats?.rank_ppr ?? 999,
...
function findLeagueType(tradeValueTable) {
  const targetSheetName = "redraft___1qb_ppr";

  const leagueCollection = tradeValueTable.playersListsCollections.find(
    (collection) => collection.sheetName === targetSheetName
  );

  if (leagueCollection) {
    console.log(`[SUCCESS] Found league type: ${targetSheetName}`);
    return leagueCollection.playersList;
  } else {
    console.error(`[ERROR] League type '${targetSheetName}' not found.`);
    return null;
  }
}
...
async function loadPlayers() {
  let allPlayerData = await fs.readFile(
    "C:/src/Fantasy-Web-Server/nfl.json",
    "utf8"
  );
  return JSON.parse(allPlayerData);
}
async function filterPlayers(data) {
  const relevantPositions = ["WR", "RB", "QB", "TE", "K"];

  return Object.values(data).filter(player => 
    player.depth_chart_order !== null && // Ensure depth_chart_order is not null
    player.depth_chart_order <= 3 &&
    player.search_rank < 500 && // Ensure search_rank is less than 2000
    player.fantasy_positions && // Ensure fantasy_positions is not null or undefined
    player.fantasy_positions.some(position => relevantPositions.includes(position))
  );
}
